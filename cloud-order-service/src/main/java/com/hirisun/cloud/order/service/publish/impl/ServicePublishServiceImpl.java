 ...
 */
@Service
public class ServicePublishServiceImpl extends ServiceImpl<ServicePublishMapper, 
	ServicePublish> implements IServicePublishService {
    ...
    @Override
    public void save(UserVO user, ServicePublishVo info) {
        info.setId(null);
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void saveMY(ServicePublishVo info) {
        info.setId(null);
        this.save(info);
        // 关联文件信息
        filesService.refFiles(info.getFileList(), info.getId());
    ...
    public ServicePublishVo getDetails(String id) {
        ...
        // 审核信息
        List<AppReviewInfoVo> allReviewInfo = appReviewInfoService.getAllReviewInfoByAppInfoId(id);
        servicePublish.setReviewList(allReviewInfo);
        // 实施审批信息
        AppReviewInfoVo implInfo = null;
        AppReviewInfoVo lastReviewInfo = appReviewInfoService.getLastPassReviewInfoByAppInfoId(id);
    ...
    @Override
    public ServicePublishVo getDetailsMY(String id) {
        ServicePublish servicePublish = this.getById(id);
        if (servicePublish == null) {
            throw new BaseException("该申请不存在");
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void update(UserVO user, ServicePublishVo info) {
        info.setStatus(null);
    ...
    @Override
    public ServicePublishVo getByActId(String activityId) {
    ...
    @Override
    public ResponseResult servicePublishApply(User user,QueryVO queryVO){
