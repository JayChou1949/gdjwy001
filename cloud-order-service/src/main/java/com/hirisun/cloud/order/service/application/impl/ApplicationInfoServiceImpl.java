package com.hirisun.cloud.order.service.application.impl;
...
import com.hirisun.cloud.order.mapper.application.ApplicationInfoMapper;
...
import com.upd.hwcloud.bean.vo.applicationInfoOrder.WorkflowApplyExportVo;
...
import com.upd.hwcloud.service.IServiceAlterService;
 ...
 */
@Service
public class ApplicationInfoServiceImpl extends ServiceImpl<ApplicationInfoMapper, 
	ApplicationInfo> implements IApplicationInfoService {
    ...
    @Autowired
    private ServiceAlterService serviceAlterService;
    ...
    public <I> void saveImpl(User user, Map<String, Object> param, IImplHandler<I> implHandler, String modelId) throws Exception {
        ...
        if ("0".equals(result)) {
            ...
            if (daasAutoEnable) {
                FormNum formNum = FormNum.getFormNumByInfo(info);
                if (formNum == FormNum.DAAS) { // daas 自动订购
                    //daas只有数据服务(DaaS)才自动订购，数据资源(DaaS)不需要订购
                    if (StringUtils.equals("数据服务(DaaS)",info.getServiceTypeName())) {
                        daasApigService.orderService(info);
                    }
                }
            }
...
//                    paasApigService.addTYXXMessage(info);
    ...
    public List<SaasApplicationExport> saasApplicationExport(String areas, String policeCategory, Map<String, String> params) {
        List<SaasApplicationExport> list = applicationInfoMapper.saasApplicationExport(areas, policeCategory, params);
        //sql获取的环节信息在驳回时有问题，需要重新验证当前待办环节,通过实例获取环节信息
        list.forEach(item->{
            Instance instance = instanceService.getInstanceByBusinessId(item.getIds());
            if(instance!=null){
                List<Activity> activitys = activityService.list(new QueryWrapper<Activity>().lambda()
                        .eq(Activity::getInstanceid,instance.getId()).eq(Activity::getActivitystatus,"待办"));
                if (activitys != null && activitys.size() > 0) {

                    Workflowmodel workflowmodel=workflowmodelService.getById(activitys.get(0).getModelid());
                    if(workflowmodel!=null){
                        item.setStepName(workflowmodel.getModelname());//重新插入当前待办环节
                    }
                }
            }
        });
    ...
    @Override
    public List<EpidemicApplicationExport> epidemicApplicationExport(String areas, String policeCategory, Map<String, String> params) {
        List<EpidemicApplicationExport> list = applicationInfoMapper.epidemicApplicationExport(areas, policeCategory, params);
        //sql获取的环节信息在驳回时有问题，需要重新验证当前待办环节,通过实例获取环节信息
        list.forEach(item->{
              Instance instance = instanceService.getInstanceByBusinessId(item.getIds());
            if(instance!=null){
    ...
    @Override
    public List<WorkflowApplyExportVo> getIPDApplicationInfoOrderList(Map<String, Object> params) {
        List<WorkflowApplyExportVo> list = applicationInfoMapper.getIPDApplicationInfoOrderList(params);
        List<WorkflowApplyExportVo> result = new ArrayList<>();
        int count = 1;
        for (WorkflowApplyExportVo ipdVo : list) {
           ...
           // String node = applicationInfoMapper.getCurrentNodeById(id);
            ...
            if(model!=null){
                ...
                //本单位审批
                ReviewInfoVo reviewInfoVo1 = appReviewInfoService.getReviewInfoVoByAppInfoId("本单位审批", id);
                ...
                Map<String,ReviewInfoVo> map=new LinkedHashMap<>();//用于数据去重
                List<ReviewInfoVo> infoList = appReviewInfoService.getDynamicReviewInfoVoByAppInfoId(id);
                infoList.forEach(item->{
                    map.put(item.getModelName(),item);
                });
                LinkedList<ReviewInfoVo> linklist= new LinkedList<>();
                for(ReviewInfoVo value : map.values()){
                    linklist.add(value);
                }
                for(int i=0;i<linklist.size();i++){
                    ReviewInfoVo info=linklist.get(i);
                    if(i==(linklist.size()-1)
                            && ((info.getModelName()!=null&&info.getModelName().equals("业务办理")) || (info.getModelType()!=null&&info.getModelType().equals("2")))){//最后一个是业务办理
                        ipdVo.setAuditTime4(info.getCreateTime());
                        ipdVo.setAuditResult4(info.getResult());
                        ipdVo.setAuditPerson4(info.getName());
                    }else {
                        switch (i) {
                            case 0:
                                ipdVo.setModelName0(info.getModelName());
                                ipdVo.setAuditTime0(info.getCreateTime());
                                ipdVo.setAuditResult0(info.getResult());
                                ipdVo.setAuditOpnion0(info.getRemark());
                                ipdVo.setAuditPerson0(info.getName());
                                break;
                            case 1:
                                ipdVo.setModelName1(info.getModelName());
                                ipdVo.setAuditTime1(info.getCreateTime());
                                ipdVo.setAuditResult1(info.getResult());
                                ipdVo.setAuditOpnion1(info.getRemark());
                                ipdVo.setAuditPerson1(info.getName());
                                break;
                            case 2:
                                ipdVo.setModelName2(info.getModelName());
                                ipdVo.setAuditTime2(info.getCreateTime());
                                ipdVo.setAuditResult2(info.getResult());
                                ipdVo.setAuditOpnion2(info.getRemark());
                                ipdVo.setAuditPerson2(info.getName());
                                break;
                            case 3:
                                ipdVo.setModelName3(info.getModelName());
                                ipdVo.setAuditTime3(info.getCreateTime());
                                ipdVo.setAuditResult3(info.getResult());
                                ipdVo.setAuditOpnion3(info.getRemark());
                                ipdVo.setAuditPerson3(info.getName());
                                break;
                        }
                    }
                }
    ...
    /**
     * 查询当前用户是否有桌面云订单（除已删除状态）
     * @param creator:身份证号
     * @return
     */
    public Integer isOrNotCloudDesktop(String creator) {
        return applicationInfoMapper.isOrNotCloudDesktop(creator);
    }
