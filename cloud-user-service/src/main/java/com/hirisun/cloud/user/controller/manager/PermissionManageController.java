...
public class PermissionManageController {
    ...
    public QueryResponseResult<Permission> list() {
        ...
        List<Permission> permissionList=permissionService.listWithTree(list);
        return QueryResponseResult.success(permissionList);
    }

    @ApiOperation("角色权限列表")
    @GetMapping("/list/{role}")
    public QueryResponseResult<Permission> listOfRole(@ApiParam("角色id") @PathVariable String role) {
        Map map = new HashMap<>();
        map.put("rid",role);
    ...
    @ApiOperation("修改角色权限")
    @PostMapping("/edit/{role}/{permission}")
    public QueryResponseResult edit(@ApiParam("角色id") @PathVariable String role,
                                    @ApiParam("权限id,多个使用逗号分隔") @PathVariable String permission) {
        /*
         * 1.移除角色权限
         * 2.批量保存角色权限
         */
    ...
    public QueryResponseResult<Permission> menu(@LoginUser UserVO user){
        ...
        List<Permission> permissionList=permissionService.listWithTree(menu);
        return  QueryResponseResult.success(permissionList);
    }
    ...
    public QueryResponseResult<Permission> delete(@PathVariable String id){
        /**
         * 1.删除菜单
         * 2.删除子节点
         */
