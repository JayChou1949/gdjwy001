...
public class PaasZhzyServiceImpl01 extends ServiceImpl<PaasZhzyMapper01, PaasZhzy> implements IPaasZhzyService01 {
    ...
    public  Double maxEsMemary(String appName, String area, String police, Integer day) {
        List<PaasZhzy> maxMemory =paasZhzyMapper01.maxEsMemary(appName,area,police,day);
        List<Double> list=new ArrayList();
        //es无内存使用率  需要通过计算获得
        for (PaasZhzy paasZhzy : maxMemory) {
            Double elasticsearchMemoryTotal = paasZhzy.getElasticsearchMemoryTotal();
            Double elasticsearchMemoryUsed = paasZhzy.getElasticsearchMemoryUsed();
            if(elasticsearchMemoryTotal==0.0){
                list.add(0.0);
            }else{
                double maxMerary= elasticsearchMemoryUsed / elasticsearchMemoryTotal;
                list.add(maxMerary);
            }
        }
    ...
    public PaasZhzy maxEsCpu(String appName, String area, String police, Integer day) {
        return paasZhzyMapper01.maxEsCpu(appName,area,police,day);
    }
