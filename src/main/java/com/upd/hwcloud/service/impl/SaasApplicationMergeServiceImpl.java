...
public class SaasApplicationMergeServiceImpl extends ServiceImpl<SaasApplicationMergeMapper, SaasApplicationMerge> implements ISaasApplicationMergeService {
    ...
    @Override
    public List<SaasTotal> saasTotal(String areas, String policeCategory, Map<String, String> params) {
        List<SaasTotal> data = baseMapper.saasTotal(areas, policeCategory, params);
        for (int i = 0; i < data.size(); i++) {
            SaasTotal st = data.get(i);
            st.setId(i + 1);
    ...
    @Override
    public List<SaasOrderTotal> saasOrderTotal(String areas, String policeCategory, Map<String, String> params) {
        List<SaasOrderTotal> data = baseMapper.saasOrderTotal(areas, policeCategory, params);
        for (int i = 0; i < data.size(); i++) {
            SaasOrderTotal st = data.get(i);
            st.setId(i + 1);
    ...
    @Override
    public List<SaasOrderTotal> saasOrderQuery(String areas, String policeCategory, Map<String, String> params) {
        List<SaasOrderTotal> data = baseMapper.saasOrderQuery(areas, policeCategory, params);
        for (int i = 0; i < data.size(); i++) {
            SaasOrderTotal st = data.get(i);
            st.setId(i + 1);
            if ("0".equals(st.getApplyType())) {
    ...
    @Override
    public List<SaasUseTotal> saasUseTotal(String areas, String policeCategory, Map<String, String> params) {
        List<SaasUseTotal> data = baseMapper.saasUseTotal(areas, policeCategory, params);
        for (int i = 0; i < data.size(); i++) {
            SaasUseTotal st = data.get(i);
            st.setId(i + 1);
            if ("0".equals(st.getApplyType())) {
                st.setAreasPoliceCategory(st.getAreas());
