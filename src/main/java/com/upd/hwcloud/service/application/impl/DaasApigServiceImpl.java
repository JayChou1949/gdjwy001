...
@Service
public class DaasApigServiceImpl implements IDaasApigService {
    ...
    public void orderService(ApplicationInfo info) throws Exception {
        List<DaasApplication> serverList = daasApplicationService.getUnsucessByAppInfoId(info.getId());
        this.order(info, serverList);
    }

    @Transactional(propagation = Propagation.NOT_SUPPORTED)
    @Override
    public void orderService(String daasApplicationId) throws Exception {
        DaasApplication daasApplication = daasApplicationService.getById(daasApplicationId);
        if (daasApplication == null) {
            throw new BaseException("id不存在");
        }
        if (StringUtils.isNotEmpty(daasApplication.getAppKey())
                && StringUtils.isNotEmpty(daasApplication.getAppSecret())) {
            throw new BaseException("该服务已经订购成功");
        }
        ApplicationInfo info = applicationInfoService.getById(daasApplication.getAppInfoId());
        this.order(info, Collections.singletonList(daasApplication));
    }

    private void order(ApplicationInfo info, List<DaasApplication> serverList) throws Exception {
        FormNum formNum = FormNum.getFormNumByInfo(info);
        if (formNum != FormNum.DAAS) {
            throw new BaseException("必须为DaaS申请");
        }
        ...
        for (DaasApplication daasApplication : serverList) {
            if (isBatch){
                try {
                    isBatch(daasApplication,userClients,info,managerClient);
                }catch (Exception e){
                    logger.error("订购失败的ID:{}",daasApplication.getId());
                    isAllSuccess=false;
                }
            }else {
                isBatch(daasApplication,userClients,info,managerClient);
    ...
    private void isBatch(DaasApplication daasApplication,List<UserClient> userClients,ApplicationInfo info,OkHttpClient managerClient) throws Exception{
        String serviceId = daasApplication.getServiceId();
        if (StringUtils.isEmpty(serviceId)) {
            daasApplication.setErrorMsg("服务ID为空");
            daasApplication.setErrorJson("");
        ...
        look: for (UserClient userClient : userClients) {
            AppList appList = this.getUserAppList(userClient.getClient(),daasApplication);
        ...
        // 4.查询套餐信息(略)
        // 5.获取需要订购的服务详情
        ServiceDetail serviceDetail = this.getServiceDetail(okHttpClient, serviceId,daasApplication);
        if (existsApp == null) {
            // 6.不存在,创建应用
            AppDetail appDetail = this.createApp(okHttpClient, info,daasApplication);
            existsApp = appDetail;
        }
        // 7.订购服务
        ServiceInstance orderInstance = this.orderService(okHttpClient, info, serviceDetail, currentUser, existsApp,daasApplication);
        // 8.审核
        ServiceInstance reviewInstance = this.review(managerClient, okHttpClient, orderInstance,daasApplication);
        //进行绑定操作时需等待实例进入运行状态
        boolean flag = true;
        int count = 0;
        ...
        do {
            ...
            //9.绑定
            try {
                serviceBinding = this.binding(okHttpClient, reviewInstance, currentUser, serviceDetail, daasApplication);
                flag = false;
                daasApplication.setAppKey(serviceBinding.getCredentials().getAppKey());
                daasApplication.setAppSecret(serviceBinding.getCredentials().getAppSecret());
                //成功后将错误信息置空
                daasApplication.setErrorMsg("");
                daasApplication.setErrorJson("");
                daasApplication.updateById();
        ...
        // 获取api列表
        this.saveUserData(okHttpClient, daasApplication, reviewInstance.getInstance_guid());
    }
    ...
    private ServiceBindingResponse binding(OkHttpClient userClient, ServiceInstance reviewInstance,
                                           ...
                                           DaasApplication daasApplication) throws Exception {
    ...
    //todo:管理员能够删除普通用户的实例?
    private ServiceInstance review(OkHttpClient managerClient, OkHttpClient userClient, ServiceInstance orderInstance,DaasApplication daasApplication) throws Exception {
    ...
    private ServiceInstance orderService(OkHttpClient client, ApplicationInfo info,
                                         ...
                                         AppDetail existsApp, DaasApplication daasApplication) throws Exception {
        ...
        if (plans == null || plans.isEmpty()) {
            daasApplication.setErrorMsg("服务套餐为空");
            daasApplication.setErrorJson("");
            daasApplication.updateById();
    ...
    private void saveUserData(OkHttpClient client, DaasApplication daasApplication, String instanceGuid) {
        ...
        try {
            ...
            daasApplication.setUserData(userData);
            daasApplication.updateById();
        } catch (Exception e) {
    ...
    private AppDetail createApp(OkHttpClient client, ApplicationInfo info,DaasApplication daasApplication) throws Exception {
    ...
    private AppList getUserAppList(OkHttpClient client,DaasApplication daasApplication) throws Exception {
        String url = apigBaseUrl + "/v1/apigw/extension/apps";
    ...
    private ServiceDetail getServiceDetail(OkHttpClient client, String serviceId,DaasApplication daasApplication) throws Exception {
