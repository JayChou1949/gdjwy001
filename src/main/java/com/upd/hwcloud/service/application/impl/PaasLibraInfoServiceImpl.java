package com.upd.hwcloud.service.application.impl;

import com.upd.hwcloud.bean.entity.application.paas.libra.PaasLibraInfo;
import com.upd.hwcloud.dao.application.PaasLibraInfoMapper;
import com.upd.hwcloud.service.application.IPaasLibraInfoService;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import org.springframework.stereotype.Service;

/**
 * <p>
 *  服务实现类
 * </p>
 *
 * @author yyc
 * @since 2020-06-11
 */
@Service
public class PaasLibraInfoServiceImpl extends ServiceImpl<PaasLibraInfoMapper, PaasLibraInfo> implements IPaasLibraInfoService {

}
