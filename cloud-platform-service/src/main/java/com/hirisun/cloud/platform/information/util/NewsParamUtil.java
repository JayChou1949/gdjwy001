package com.hirisun.cloud.platform.information.util;
...
public class NewsParamUtil {
    ...
    public static boolean checkUserInfomationPermission(UserVO user, Integer provincial, String belong) {
        ...
        if (provincial.equals(NewsParamUtil.NEWS_TYPE_PROVINCE)
                && !user.getType().equals(UserVO.USER_TYPE_PROVINCIAL_MANAGER)) {
            return true;
        }
        if (provincial.equals(NewsParamUtil.NEWS_TYPE_AREA)
                && (StringUtils.isEmpty(belong) || !belong.equals(user.getTenantArea()))) {
            return true;
        }
        if (provincial.equals(NewsParamUtil.NEWS_TYPE_POLICE)
                && (StringUtils.isEmpty(belong) || !belong.equals(user.getTenantPoliceCategory()))) {
            return true;
        }
        if (provincial.equals(NewsParamUtil.NEWS_TYPE_NATIONAL_PROJECT)
